...
```cron
5 * * * * echo "远航最帅!" >> ~/data/hello.txt
```

`/var/log/cron` 中只有 **21:05:01** 执行过一次"远航最帅"，之后 21:10、21:20、21:30、21:40 均无记录——任务没有按预期的"每 5 分钟"执行。

## 二、排查过程

| 步骤 | 命令 | 结果 | 结论 |
| --- | --- | --- | --- |
| ① 查服务 | `systemctl status crond` | `active (running)`，enabled，PID 9288 运行中 | 服务正常，排除 |
| ② 查任务 | `crontab -l` | 第一颗星是 `5` 而不是 `*/5` | 锁定时间表达式 |
| ③ 查日志 | `tail -n 10 /var/log/cron` | 仅 21:05 一次，其余是 sa1 系统任务 | 证实每小时只跑一次 |
...
```text
5 * * * *      # 每小时第 5 分钟执行（一天 24 次）❌
*/5 * * * *    # 每 5 分钟执行（一天 288 次）✅
```

第一颗星写成了固定值 `5`，导致执行频率错误。
...
```bash
cat /root/data/hello.txt     # 应持续增加
tail -n 5 /var/log/cron      # 应每 5 分钟出现一次
```

## 经验总结

**服务正常 ≠ 任务正确**。排查定时任务三步走：先确认 crond 在跑，再检查时间表达式，最后用 `/var/log/cron` 日志验证实际执行情况。
